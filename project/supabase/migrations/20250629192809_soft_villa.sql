/*
  ...
  3. Functions
    - `create_password_reset_token` - Creates a new reset token
    - `validate_password_reset_token` - Validates a token
    - `handle_password_reset` - Updates password and invalidates tokens
    - `request_password_reset` - Handles reset request flow
...
-- Create policies (check if they exist first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'password_reset_tokens' 
    AND policyname = 'Users can view their own reset tokens'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'trigger_cleanup_expired_tokens'
  ) THEN
...
EXCEPTION
  WHEN OTHERS THEN
    -- If there's an error checking for the trigger, we'll try to drop and recreate it
    DROP TRIGGER IF EXISTS trigger_cleanup_expired_tokens ON password_reset_tokens;
    CREATE TRIGGER trigger_cleanup_expired_tokens
    AFTER INSERT ON password_reset_tokens
    FOR EACH STATEMENT
    EXECUTE FUNCTION cleanup_expired_tokens();
