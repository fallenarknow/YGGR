...
import { SellerSubscriptionBanner } from '../components/SellerSubscriptionBanner';

export const SellerPlantNew: React.FC = () => {
  const { user } = useAuth();
  const { hasAccess } = useSubscription();
  const navigate = useNavigate();
  
  const [plantCount, setPlantCount] = useState(0);
  const [canAddPlant, setCanAddPlant] = useState(true);
  ...
  // Check if seller can add more plants
  useEffect(() => {
    const checkPlantLimit = async () => {
      ...
      try {
        ...
        // Check if seller can add more plants
        const canAdd = await hasAccess('plantListings', (count || 0) + 1);
        setCanAddPlant(canAdd);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent, publishImmediately = false) => {
    ...
    if (!canAddPlant) {
      setError('You have reached your plant listing limit. Please upgrade your subscription to add more plants.');
      return;
    }
    ...
    try {
      ...
      for (const image of images) {
        ...
        altTexts.push(formData.name); // Default alt text is plant name
      }
      
      // Create plant record in database
      ...
      setTimeout(() => {
        if (publishImmediately) {
          ...
          // Reset form for adding another plant
          setFormData({
            name: '',
            botanical_name: '',
            price: '',
          ...
          // Check if seller can add more plants
          hasAccess('plantListings', plantCount + 2).then(setCanAddPlant);
        }
      }, 2000);
    } catch (error: any) {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-secondary-900">Add New Plant</h1>
              <p className="text-secondary-600 text-lg">Create a new plant listing for your inventory</p>
            </div>
          </div>
        </div>
        ...
        {/* Subscription Banner */}
        {!canAddPlant && (
          <SellerSubscriptionBanner 
            feature="plantListings" 
            quantity={plantCount + 1} 
            className="mb-6"
          />
        )}
        ...
        {success && (
          <div className="mb-6 bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center space-x-3 animate-fade-in">
            ...
            <span className="text-green-800 font-medium">Plant added successfully!</span>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <form onSubmit={(e) => handleSubmit(e, false)}>
            ...
            <div className="bg-secondary-50 p-6 sm:p-8 flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
              ...
              <button
                type="submit"
                disabled={isSubmitting || !canAddPlant}
